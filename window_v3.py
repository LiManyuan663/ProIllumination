...
class ProIllumination():
    def __init__(self, image_root, args):
        ...
        self.frame_buttons = Frame(self.master, height=720, width=200, relief=RIDGE, bg='white', bd=5, borderwidth=4)
        self.frame_buttons.pack(side="left", anchor=N, fill=Y, ipadx=2, expand=False)

        self.frame_label = Frame(self.master, relief=RIDGE, bg='grey', bd=5, borderwidth=4)
        self.frame_label.pack(side=LEFT, anchor=N, fill=BOTH, expand=True)

        self.canves = Canvas(self.frame_label)
    ...
    def init_buttons(self):
        read_image_button = Button(self.frame_buttons, text='Read Image', command=self.update_raw_image)
        read_image_button.pack(side="top", anchor=N, pady=5, fill=X)

        reload_image_button = Button(self.frame_buttons, text='Reload Image', command=self.reload_image_seg)
        reload_image_button.pack(side="top", anchor=N, pady=5, fill=X)

        shadow_detect_button = Button(self.frame_buttons, text='Detect Shadow')
        ...
        save_image_button = Button(self.frame_buttons, text='Save Result')
        save_image_button.pack(side="top", anchor=N, pady=5, fill=X)
        ...
        quit_button.pack(side="bottom", anchor=S, pady=5)
    ...
    def update_raw_image(self):
        ...
        if file_path:
            ...
            # 更新seg_result变量，以便后续保存
            self.seg_result = raw_image_tk
